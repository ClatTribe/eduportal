...
import { FilterValues, emptyFilters } from "../../../components/CourseFinder/types";
...
const CourseFinder: React.FC = () => {
  ...
  useEffect(() => {
    if (viewMode === "all") {
      fetchCourses(currentPage, activeFilters);
    }
  }, [viewMode, currentPage, activeFilters]);
  ...
  const fetchCourses = async (page: number, filters: FilterValues) => {
    try {
      ...
      if (filters.studyLevel) {
        query = query.eq("Study Level", filters.studyLevel);
      }
  ...
  const handleFilterValuesChange = (filters: FilterValues) => {
  ...
  const displayCourses = viewMode === "recommended"
    ? recommendedCourses.slice(currentPage * perPage, (currentPage + 1) * perPage)
    : courses;
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        ...
                        <div className="flex items-center gap-3 sm:gap-4 pt-3 sm:pt-4">
                          ...
                          <a
                            ...
                            className="flex-1 bg-green-600 text-white rounded-lg py-2 sm:py-2.5 px-3 sm:px-4 hover:bg-green-700 transition-colors flex items-center justify-center gap-2 text-xs sm:text-sm font-semibold shadow-sm"
                          >
